...
#	-text "Bajo nivel (en profundidad)" -value profundidad_bajo -command {RadioButtonsProc}
...
proc Plan {} {
	...
	return
        ...
        $blq(listbox) insert 0 "Calculando..."
	$blq(botonConfigurarInicial) config -state disabled
	$blq(botonConfigurarFinal) config -state disabled
...
proc Log {} {
	...
	$blq(listbox) delete 0
	if {($blq(numPasos) != 0) &&
	    ([string compare $blq(algoritmo) "anchura_bajo"] == 0 ||
	     [string compare $blq(algoritmo) "profundidad_bajo"] == 0)} then {
	    $blq(botonEjecutar) config -state normal
	}
	$blq(botonConfigurarInicial) config -state normal
	$blq(botonConfigurarFinal) config -state normal
